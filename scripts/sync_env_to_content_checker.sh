#!/bin/bash
#
# Sync environment variables from .env.production to Railway (Content Checker Backend) via GraphQL API
# Uses variableCollectionUpsert for batch updates (avoids rate limits)
#
# Usage:
#   ./scripts/sync_env_to_content_checker.sh [project_id] [environment_id] [service_id]
#
# Arguments:
...
ENV_FILE="$PROJECT_ROOT/programs/content_checker_backend/.env.production"
...
if [ ! -f "$ENV_FILE" ]; then
    log_error "Content Checker Backend .env.production not found!"
    log_info "Expected location: programs/content_checker_backend/.env.production"
    log_info "Create it by copying the example:"
    echo "  cp programs/content_checker_backend/.env.production.example \\"
    echo "     programs/content_checker_backend/.env.production"
...
log_info "Syncing to Railway (Content Checker Backend)..."
...
cat > /tmp/railway_sync.py << 'PYTHON'
import os
import json
import sys
import subprocess
...
# Add all variables (passed as environment variables)
add_var("SUPABASE_URL", os.getenv("SUPABASE_URL", ""))
add_var("SUPABASE_SERVICE_ROLE_KEY", os.getenv("SUPABASE_SERVICE_ROLE_KEY", ""))
...
python3 /tmp/railway_sync.py

# Cleanup
rm -f /tmp/railway_sync.py /tmp/mutation.json
